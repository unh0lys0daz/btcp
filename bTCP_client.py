...
def connect(dest_ip, dest_port):
    ...
    # syn packet creation
    pseudo_header_syn = pack(header_format, stream_id, 0, 0, BTCP_SYN, 255, 1000, 0)
    bTCP_header_syn = pack(header_format, stream_id, 0, 0, BTCP_SYN, 255, 1000, calculate_checksum(pseudo_header_syn))

    syn_packet = bTCP_header_syn
    # send syn
    ...
    if calculate_checksum(pseudo_header_synack) != synack_tuple[6]:
        print("Checksum synack doesn't match")


  # ack creation
    pseudo_header_ack = pack(header_format, stream_id, ack_nr_synack, syn_nr_synack+1, BTCP_ACK, window_synack, size_synack, 0)
